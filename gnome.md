...
GNOME with a few additional packages for a base system according to my personal preferences.

```bash
sudo pacman -S gnome gnome-tweaks xdg-utils pipewire pipewire-audio pipewire-pulse pipewire-jack openssh
 ```

## Configure start GNOME

```bash
sudo systemctl enable gdm.service
```
Reboot (should start GNOME automatically after logging)
